...
@property (nonatomic, strong) NSMutableDictionary *remoteFileRevs;
...
- (void)showWorking {
    ...
    int ht = 30;
    workingLabel.frame = CGRectMake(135, 600-ht, 50, ht);
    ...
    [UIView animateWithDuration:0.3 animations:^{
        workingLabel.frame = CGRectOffset(workingLabel.frame, 0, -40);
    }];
}
...
- (void)hideWorking {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        workingLabel.frame = CGRectMake(workingLabel.frame.origin.x, workingLabel.frame.origin.y + 40, workingLabel.frame.size.width, workingLabel.frame.size.height);
...
- (NSDictionary *)getLocalStatus {
    ...
    for (NSString* item in [fm subpathsOfDirectoryAtPath:root error:nil])
    {
        ...
        NSDictionary *attribs = [fm attributesOfItemAtPath:itemPath error:nil];
...
- (void)restClient:(DBRestClient*)_client loadedMetadata:(DBMetadata*)metadata {    
    for (DBMetadata *item in metadata.contents) {
        ...
        {
            [self.remoteFiles setObject:item.lastModifiedDate forKey:[self noSlash:item.path]];
            [self.remoteFileRevs setObject:item.rev forKey:[self noSlash:item.path]];
        }
    }
    
    NSLog(@"remote files: %@", self.remoteFiles);
    
    // Now do the comparisons to figure out what needs doing
    BOOL allComplete = [self syncStepWithRemoteFiles:self.remoteFiles andRevs:self.remoteFileRevs];
